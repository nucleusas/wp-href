...
(function ($) {
  ...
  /**
   * Reset the rebuild modal to its initial state
   */
  function resetRebuildModal() {
    // Reset progress bars to 0%
    $siteProgress.css("width", "0%");
    $postProgress.css("width", "0%");
    ...
    // Reset labels from "Complete" back to progress labels
    ...
    $startButton
      .show()
      .prop("disabled", false)
      .text(settings.i18n.start_rebuild);
    $cancelButton.show();
    $closeButton.hide();
  }
  ...
  function saveSettings() {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        // Reset and show rebuild modal
        resetRebuildModal();

        // Setup the modal text
   ...
   *
   * @param {number} retryCount Number of retries attempted for this batch
   */
  function processNextBatch(retryCount = 0) {
    // Maximum number of retries for a single batch
    const MAX_RETRIES = 3;
    ...
    $.ajax({
      ...
      success: function (response) {
        if (!response.success) {
          ...
          if (retryCount < MAX_RETRIES) {
            ...
            setTimeout(() => processNextBatch(retryCount + 1), 1000);
            return;
          }
      ...
      error: function (error) {
        ...
        if (retryCount < MAX_RETRIES) {
          console.log(
            `Network error, retrying batch (${
              retryCount + 1
            }/${MAX_RETRIES})...`
          );
          setTimeout(
            () => processNextBatch(retryCount + 1),
            1000 * (retryCount + 1)
          ); // Exponential backoff
          return;
        }
  ...
  function completeRebuild() {
    ...
    // Hide action buttons and show close button
    $startButton.hide();
